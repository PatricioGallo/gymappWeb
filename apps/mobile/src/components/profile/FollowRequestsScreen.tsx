...
import { acceptFollowRequest, listFollowRequests, rejectFollowRequest, type FollowRequestRow } from "@/lib/followService";
...
export function FollowRequestsScreen() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      setRows(await listFollowRequests(myId));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleRespond(row: FollowRequestRow, accept: boolean) {
    setBusyId(row.id);
    const { error } = accept ? await acceptFollowRequest(row.id) : await rejectFollowRequest(row.id);
    setBusyId(null);
    if (error) return;
  ...
  return (
    <View style={styles.flex}>
      <ScreenHeader title="Solicitudes de seguimiento" onBack={() => router.back()} avatarUrl={myAvatarUrl} />
      ...
      {loading ? (
        ...
        <View style={styles.listWrap}>
          <Text style={styles.summary}>
            {rows.length === 0 ? "No tenés solicitudes de seguimiento pendientes." : `${rows.length} solicitud${rows.length === 1 ? "" : "es"} pendiente${rows.length === 1 ? "" : "s"}.`}
          </Text>
          {rows.map((r) => (
