        ...
        .map((d: any) => ({
          ...
          ejercicios: [...(d.routine_exercises ?? [])]
            ...
            .map((e: any) => {
              ...
              const author = exercises?.is_builtin ? "Gym Social" : exercises?.profiles ? `${exercises.profiles.nombre} ${exercises.profiles.apellido}` : "Gym Social";
              return { ...rest, authorName: author, category: exercises?.category ?? null, image_url: exercises?.image_url ?? null };
            }),
        })),
    }));
...
export async function updateRoutineExercise(
  id: string,
  fields: Partial<Pick<RoutineExercise, "serie" | "repe" | "repe_max" | "nota" | "es_medible" | "mismo_peso" | "orden" | "exercise_id" | "nombre_snapshot" | "info_snapshot">>
): Promise<void> {
...
export async function addRoutineExercise(dayId: string, input: NewDayInput["ejercicios"][number]): Promise<void> {
