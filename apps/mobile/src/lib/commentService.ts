...
export async function getTodayComments(routineExerciseIds: string[], fecha: string): Promise<Map<string, ExerciseComment>> {
  ...
  const { data, error } = await supabase.from("exercise_comments").select("*").in("routine_exercise_id", routineExerciseIds).eq("fecha", fecha);
...
/** Ultimo comentario (cualquier fecha) por routine_exercise_id. */
