...
const VIEW_MARKUP = `
  <section class="page-hero">
    <div class="container">
      ...
      <span class="eyebrow">Entrenar</span>
...
function unitOf(entries: LatestWeightEntry[] | undefined): WeightUnit {
  return entries && entries.length > 0 ? entries[0].unidad : "kg";
}

// mount() define render() (arma el DOM entero desde cero para cada ?rid=/?uid= -- no hay estado
// fino que preservar entre rutinas distintas) y la deja referenciada aca para que update() la
...
export const pesosView: ViewModule = {
  async mount(container, params, ctx, authUserId) {
    ...
    async function render(p: URLSearchParams): Promise<void> {
      ...
      // (armado una sola vez por render(), sobre el weekContent recien creado) para saber bajo
      // qué clave guardar cada tipeo.
      ...
      // se pierda. Corre al entrar a la pantalla (y de nuevo cada vez que se re-entra via update(),
      // ver mount()), antes de traer los pesos ya guardados, para que lo recién subido ya aparezca.
      // Filas incompletas o inválidas se descartan en silencio (no hay usuario mirando para
      // corregirlas); si falla la subida, el borrador se deja intacto para reintentar la próxima vez.
      ...
      // Delegado sobre el contenedor estable de este render() (su innerHTML se reemplaza en
      // cada openDay(), pero el nodo en si no cambia hasta el proximo render()) -- evita
      // reenganchar un listener nuevo por cada input de cada ejercicio, y por cada visita a un día.
      const weekContentRoot = container.querySelector("#weekContent")!;
      weekContentRoot.addEventListener(
        "input",
        (e) => {
      ...
      function openReportErrorModal(excName?: string): void {
      ...
      function confirmDeleteWeightModal(exc: { id: string; nombre_snapshot: string }, weekIndex: number, diaIndex: number): void {
        const loaderBody = document.getElementById("loaderBody");
        if (!loaderBody) return;
        loaderBody.innerHTML = `
          <div class="success-check-container">
      ...
      function openCommentModal(exc: { id: string; nombre_snapshot: string }, weekIndex: number, diaIndex: number): void {
        const loaderBody = document.getElementById("loaderBody");
        if (!loaderBody) return;
      ...
      function renderWeekStatus(weekIndex: number): void {
      ...
      function renderWeek(weekIndex: number): void {
        const semana = routine!.semanas[weekIndex];
        const weekContent = container.querySelector("#weekContent")!;
        (weekContent as HTMLElement).dataset.week = String(weekIndex);
      ...
      function backToWeek(weekIndex: number): void {
        (container.querySelector("#weekPicker") as HTMLElement).style.display = "";
      ...
      function openDay(weekIndex: number, diaIndex: number): void {
        const semana = routine!.semanas[weekIndex];
        const dia = semana.dias[diaIndex];
        const weekContent = container.querySelector("#weekContent")!;
        (container.querySelector("#weekPicker") as HTMLElement).style.display = "none";
        ...
        if (trackable.length === 0) {
          ...
          `;
          weekContent.querySelector("#backToWeek")?.addEventListener("click", (e) => {
            e.preventDefault();
          ...
          <div class="auth-card reveal">
            ...
            ${trackable
              .map((exc, idx) => {
                ...
                const unit = exc.mismo_peso ? unitOf(history?.get(1)) : exerciseDefaultUnit(history, exc.serie);
        ...
        container.querySelector("#weightsHelpBtn")?.addEventListener("click", openWeightsHelp);

        weekContent.querySelector("#backToWeek")?.addEventListener("click", (e) => {
          e.preventDefault();
          backToWeek(weekIndex);
        });
        container.querySelector("#saveWeights")?.addEventListener("click", () => void saveWeights(weekIndex, diaIndex));
      }

      async function saveWeights(weekIndex: number, diaIndex: number): Promise<void> {
        const alertMessage = container.querySelector("#alert_message")!;
        const inputs = container.querySelectorAll<HTMLInputElement>(".weightInput");
        const today = todayLocalISO();
