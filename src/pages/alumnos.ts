...
const VIEW_MARKUP = `
  ...
  <section class="features">
    <div class="container">
      <div class="routine-tabs" id="alumnosTabs">
        <button class="routine-tab active" data-tab="current" type="button">Actuales</button>
...
export const alumnosView: ViewModule = {
  async mount(container, _params, ctx, authUserId) {
    ...
    const summaryEl = container.querySelector("#alumnosSummary")!;
    const listEl = container.querySelector("#alumnosList")!;
    ...
    function wireStudentMenus(root: HTMLElement): void {
    ...
    function renderCurrentList(students: StudentRow[], query: string): void {
      ...
      wireStudentMenus(listEl as HTMLElement);
    ...
    function confirmCancelSubscription(studentId: string, nombre: string): void {
      const loaderBody = document.getElementById("loaderBody");
    ...
    function confirmFinishActiveRoutine(routineId: string, routineNombre: string): void {
      const loaderBody = document.getElementById("loaderBody");
      if (!loaderBody) return;
      loaderBody.innerHTML = `
        <div class="success-check-container">
    ...
    function openAssignRoutineModal(studentId: string, nombre: string): void {
      const loaderBody = document.getElementById("loaderBody");
      if (!loaderBody) return;
      loaderBody.innerHTML = `
        <div class="success-check-container">
      ...
      void (async () => {
        ...
        bodyEl.querySelectorAll<HTMLButtonElement>(".assignTemplateBtn").forEach((btn) => {
          btn.addEventListener("click", () => {
            smartNavigate(`rutinsView.html?uid=${encodeURIComponent(studentId)}&cloneFrom=${encodeURIComponent(btn.dataset.id!)}`);
          });
        });
      })();
    }
    ...
    function openHistoricRoutinesModal(studentId: string, nombre: string, canReassign = true): void {
      const loaderBody = document.getElementById("loaderBody");
      if (!loaderBody) return;
      loaderBody.innerHTML = `
        <div class="success-check-container">
      ...
      void (async () => {
        ...
        bodyEl.querySelectorAll<HTMLButtonElement>(".reassignHistoricBtn").forEach((btn) => {
          btn.addEventListener("click", () => {
            smartNavigate(`rutinsView.html?uid=${encodeURIComponent(btn.dataset.uid!)}&cloneFrom=${encodeURIComponent(btn.dataset.id!)}`);
          });
        });
      })();
    }
    ...
    function renderHistoricList(students: HistoricSubscriberRow[], query: string): void {
      summaryEl.textContent =
        students.length === 0
          ? query
            ? `Sin resultados para "${query}".`
    ...
    function confirmDeleteHistoricStudent(studentId: string, nombre: string): void {
      const loaderBody = document.getElementById("loaderBody");
      if (!loaderBody) return;
      loaderBody.innerHTML = `
        <div class="success-check-container">
